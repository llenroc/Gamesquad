        ...
        public startSignalRGroup() {
            ...
            var connectionCheck = setInterval(() => {
                if ($.connection.chatHub.state !== $.signalR.connectionState.connected) {
                    $.connection.chatHub.server.joinRoom(roomName, userName).done(() => { console.log("Joined Group!") });
                    clearInterval(connectionCheck);
                }
            }, 2000)
        }

        constructor(private teamService: GameSquad.Services.TeamService, private userService: GameSquad.Services.UserService, $stateParams: ng.ui.IStateParamsService, private $scope: ng.IScope, private accountService: GameSquad.Services.AccountService) {
